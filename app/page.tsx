...
export default function Home() {
  const [rionays, setRionays] = useState<DiscordUser | null>(null);
  const [friend, setFriend] = useState<DiscordUser | null>(null);
  const [loading, setLoading] = useState(true);

  const fetchDiscordUser = async (userId: string, setter: React.Dispatch<React.SetStateAction<DiscordUser | null>>) => {
    try {
      ...
      if (data.success && data.data) {
        ...
        if (dData.activities && dData.activities.length > 0) {
          const customAct = dData.activities.find((act: any) => act.type === 4);
          if (customAct && customAct.state) {
            customStatus = customAct.state;
          } else {
            const activeGame = dData.activities.find((act: any) => act.type === 0);
  ...
  return (
    <main className="min-h-screen flex items-center justify-center p-4 md:p-8 selection:bg-rose-500 selection:text-white relative overflow-hidden bg-[#050508]">
      {/* Arka Plan Görseli */}
      <div 
